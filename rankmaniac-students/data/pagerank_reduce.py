...
# pline = [curr_pr, prev_pr, outlinks.....]
alpha = 0.85
node_pr = {}    # key: nodeID (string), value is contribution (float)
graph_data = {}     # key: nodeID (string), value is [pline = curr_pr, prev_pr, outlinks...] (list of strings)
...
for line in sys.stdin:
    ...
    if line_tab[0] == "i" or line_tab[0] == 'c':    # if line tells iteration or convergence counter
        ...
        line_info = line_tab[1].strip().split(',')          # get contribution/pline
...
for n_id in node_pr.keys():                         # iterate through nodes in pagerank dictionary
    ...
    if len(graph_data[n_id]) > 2:                  # if this node has outlinks, print outlinks
        outlinks = ','.join(graph_data[n_id][2:])       # get comma separated outlinks (without curr_pr, prev_pr)
